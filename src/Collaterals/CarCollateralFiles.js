...
import { Card, CardHeader, CardBody, CardFooter, Alert } from 'react-bootstrap';
...
function CarCollateralFiles() {
  ...
  const getCarCollateralCheckList = () => {
    axios
      .get(`http://localhost:8000/checklist/checklistbyparent/6`)
      .then((res) => {
        console.log(res.data);
        setChklst(res.data);
        setErr(false);
  ...
  const getAllFiles = () => {
    axios
      ...
      .then((res) => {
        ...
        setErr(false);
        setAllUploadIs(() => res.data.carfiles.map(({ checkListId }) => checkListId));
      })
      .then((err) => {
        console.log(err);
      });
  ...
  function handleUpload() {
    ...
    fd.append('collateralcar', collateralId);
    ...
    axios
      .post('http://localhost:8000/car_commonfiles/carfiles', fd, {
  ...
  return (
    <div className="container">
      <Alert>
        ...
        - {collateralDetail.manufacturedYear?.yearRange} - { " Insurance " + collateralDetail.insuranceValue}
      </Alert>
      ...
      <Card>
        ...
        <CardBody>
          ...
          >
            ...
            {chklst.length > 0 &&
              chklst.map((chk) => {
                return !allUploadIs.includes(chk.id) && <option value={chk.id}> {chk.enName} </option>;
              })}
          </Form.Select>
          ...
          <tbody>
            {Object.keys(chklstUploaded).length > 0 &&
              ...
              chklstUploaded.carfiles.map((upload) => {
                return (
                  <tr>
                    <td>{chklst.find((x) => x.id == upload.checkListId)?.enName}</td>
                    <td> </td>
                    <td>
